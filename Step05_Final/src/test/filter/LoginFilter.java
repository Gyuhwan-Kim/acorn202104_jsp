...
/*
 * 	@WebFilter(urlPatterns={"filtering path1","filtering path2",...})
 */
...
public class LoginFilter implements Filter{
	...
	// filter가 동작할 때 호출되는 method
	@Override
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
			throws IOException, ServletException {
		// TODO Auto-generated method stub
		
		// 1. login을 했는지 로그인한 아이디를 읽어본다.(HttpSession 객체)
		HttpServletRequest req=(HttpServletRequest)request;
		HttpSession session=req.getSession();
		String id=(String)session.getAttribute("id");
		
		if(id!=null) {
			// 2. 만약 login을 했다면 관여하지 않고 요청의 흐름을 이어간다. (FilterChain 객체)
			// 이걸 안 넣어주면 흐름이 진행이 안된다.
			...
			// 3. 로그인을 하지 않았다면 /users/loginform.jsp page로 redirect (HttpServletResponse 객체)
