	// 인자로 전달하는 문자열에 opts object에 있는 내용을 연결해서 문자열을 얻어내는 함수.
...
// 함수의 인자로 ajax 전송할 form의 참조값을 넣어주면
// 알아서 ajax 전송되도록 하는 함수
function ajaxFormPromise(form){
	const url=form.getAttribute("action");
	const method=form.getAttribute("method");
    // Promise 객체를 담을 변수 만들기 
    let promise;
    // file upload form인지 확인해서
    if(form.getAttribute("enctype")=="multipart/form-data"){
    	// form에 입력한 data를 FormData에 담고
    	let data=new FormData(form);
    	// fetch() function이 return하는 Promise 객체를
    	promise=fetch(url, {
    		method: "post",
    		body: data
    	});
    	return promise; // return해준다.(여기서 function이 종료된다.)
    }
...
// function의 인자로 요청 url과 ajax 전송할 내용이 있는
// input 요소의 참조값을 전달하면 ajax 전송해주는 function
